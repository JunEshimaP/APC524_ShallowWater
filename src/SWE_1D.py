...
def inputInitialValue(
    ...
    # traveling waves
    if choice == 3:
        h_i: FArray = numpy.array(
            [1 + 0.1 * math.sin(x / 10 * math.pi) for x in xArray]
        )
        hu_i: FArray = 3 * h_i
...
def loopIndex(vecf: FArray, deviation: int) -> FArray:
    ...
    loopindex: FArray = (
        numpy.linspace(deviation, nums + deviation - 1, num=nums).astype(int)
        - (deviation >= 0) * nums
    )
    return loopindex
...
def upwindDiff_Order1(vecu: FArray, dx: float) -> FArray:
    ...
    alpha: float = math.sqrt(g * numpy.amax(vecu[0])) + numpy.amax(
        abs(vecu[1] / vecu[0])
    )
...
def weno_Interp(f: FArray, stencil: FArray) -> FArray:
    ...
    # nonlinear weights
    IS0: FArray = 13.0 / 12.0 * pow(fm2 - 2.0 * fm1 + fe0, 2) + 1.0 / 4.0 * pow(
        fm2 - 4.0 * fm1 + 3.0 * fe0, 2
    )
    IS1: FArray = 13.0 / 12.0 * pow(fm1 - 2.0 * fe0 + fp1, 2) + 1.0 / 4.0 * pow(
        fm1 - fp1, 2
    )
    IS2: FArray = 13.0 / 12.0 * pow(fe0 - 2.0 * fp1 + fp2, 2) + 1.0 / 4.0 * pow(
        3.0 * fe0 - 4.0 * fp1 + fp2, 2
    )
    ...
    fp = (
        w0 * (2.0 / 6.0 * fm2 - 7.0 / 6.0 * fm1 + 11.0 / 6.0 * fe0)
...
def RK4(h: FArray, hu: FArray, dx: float, dt: float, SD) -> list:
    ...
    # the next step based on the flux at t + dt/2
    newvecu = vecu3 + 1.0 / 6.0 * dt * (
        -SD(vecu, dx) - 2.0 * SD(vecu1, dx) + 4.0 * SD(vecu2, dx) - SD(vecu3, dx)
    )
...
def SWE_1D(
    ...
    **kwargs,
    ...
    while t < timeLength:
        dt: float = min(0.001 * dx / math.sqrt(g * h.max()), 0.5 / FPS)
